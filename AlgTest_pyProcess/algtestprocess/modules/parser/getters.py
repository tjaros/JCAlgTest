...
from algtestprocess.modules.parser.javacard.performance import (
    ...
    PerformanceParserJC,
    create_sorted_already_measured_list,
    fix_error_codes,
    fix_missing_underscores,
    fix_missing_variable_data_lengths,
    convert_to_json,
    prepare_missing_measurements,
    compute_stats,
)
...
def get_javacard_profiles(directory, preprocess: bool):
    ...
    profiles_support = list(
        map(
            lambda x: SupportParserJC(x).parse(),
            filter(lambda y: "/results/" in y.lower(), files_support),
        )
    )
...
def get_tpm_profiles(
    ...
    performance = support = cryptoprops = []
    ...
    if parse_performance:
        ...
        performance = list(
            filter(lambda profile: profile.results, map(perf_parser, performance))
        )
    ...
    if parse_support:
        ...
        support = list(
            filter(lambda profile: profile.results, map(support_parser, support))
        )
        rename_duplicates(support)
        support = tpm_sorted(support, lambda x: x.device_name())

    if parse_cryptoprops:
        cryptoprops_paths = list(
            set(
                map(
                    lambda match: match.group(1),
                    filter(
                        None,
                        map(lambda name: re.search(r"(.*/[Dd]etail/).*", name), files),
                    ),
                )
            )
        )
...
def rename_duplicates(profiles):
    ...
    for profile in profiles:
        duplicates = list(
            filter(lambda x: x.device_name() == profile.device_name(), profiles)
        )
        if len(duplicates) > 1:
