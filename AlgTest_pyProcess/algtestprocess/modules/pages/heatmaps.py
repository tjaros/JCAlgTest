...
class Heatmaps(Page):
    ...
    def run(self, output_path: str, notebook=False):
        ...
        filtered_1024 = list(map(
            lambda profile: (profile["rsa_1024"], profile["device_name"]),
            filter(
                lambda profile: profile.get("rsa_1024") is not None,
                self.profiles
            )))
        filtered_2048 = list(map(
            lambda profile: (profile["rsa_2048"], profile["device_name"]),
            filter(
                lambda profile: profile.get("rsa_2048") is not None,
                self.profiles
            )))
